...
public class CustomRangeSeekBar extends View {
  ...
  public void initWidths(int min, int max, int duration) {
    mDuration = duration;
    mMaxWidth = mBarThumbs.get(1).getPos() - mBarThumbs.get(0).getPos();
    mMinWidth = min * mMaxWidth / (mDuration > max ? max : mDuration);
  ...
  public boolean onTouchEvent(@NonNull MotionEvent ev) {
    ...
    case MotionEvent.ACTION_MOVE: {
      ...
      if (currentThumb == 0) {
        ...
        if (mMinWidth <= 1 && (newX + mBarThumb.getWidthBitmap()) >= mBarThumb2.getPos()) {
          mBarThumb.setPos(mBarThumb2.getPos() - mBarThumb.getWidthBitmap());
        } else if (newX <= mPixelRangeMin) {
          mBarThumb.setPos(mPixelRangeMin);
          if (mBarThumb2.getPos() - newX > mMaxWidth) {
            mBarThumb2.setPos(newX + mMaxWidth);
          ...
          //check if we didnt exceed the min limit
          if (mBarThumb2.getPos() - newX < mMinWidth) {

            mBarThumb.setPos(mBarThumb2.getPos() - mMinWidth);
            mBarThumb.setLastTouchX(mBarThumb.getPos());

          } else {
            // Move the object
            mBarThumb.setPos(newX);
        ...
        if (mMinWidth <= 1 && newX <= mBarThumb2.getPos() + mBarThumb2.getWidthBitmap()) {
          mBarThumb.setPos(mBarThumb2.getPos() + mBarThumb.getWidthBitmap());
        } else if (newX >= mPixelRangeMax) {
          mBarThumb.setPos(mPixelRangeMax);
          if ((newX - mBarThumb2.getPos()) > mMaxWidth) {
            mBarThumb2.setPos(newX - mMaxWidth);
          ...
          //check if we didnt exceed the min limit
          if ((newX - mBarThumb2.getPos()) > mMinWidth) {
            // isMin=false;
            // Move the object
            mBarThumb.setPos(newX);
            // Remember this touch position for the next move event
